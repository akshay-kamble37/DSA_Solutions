...
class Solution {
  public:
    vector<int> reducePairs(vector<int>& num) {
        // code here
        vector<int> ans;
        
        for(int i=0;i<num.size();i++){
            
            if(ans.empty()) ans.push_back(num[i]);
            
            else if((ans.back() >= 0 && num[i] >= 0) || (ans.back() < 0 && num[i] < 0)) ans.push_back(num[i]);
            
            else if(ans.back() >= 0 && num[i] < 0){

                while(! ans.empty() && ans.back() >= 0 && ans.back() < abs(num[i])){
                    ans.pop_back();
                }
                if(! ans.empty() && ans.back() ==  abs(num[i])){
                    ans.pop_back();
                }
                else if(ans.empty()){
                    ans.push_back(num[i]);
                ...
                while(! ans.empty() && ans.back() <= 0 &&  abs(ans.back()) < (num[i])){
                    ans.pop_back();
                }
                if(! ans.empty() && abs(ans.back()) == (num[i])){
                    ans.pop_back();
                }
                else if(ans.empty()){
                    ans.push_back(num[i]);
